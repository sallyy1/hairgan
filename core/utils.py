...
#from StarGAN_v2.test import make_img # 사용 안함
...
def translate_using_reference_sample(nets, args, x_src, x_ref, x_mask, y_ref, img_cnt, total_len, img_name): # 이거 사용
    ...
    # hyun 추가 (생성된 이미지와 src 이미지를 Face_parsing 모델과 SEAN 모델을 거침)
    # x_fake 와 x_src 사용
    parsing(img=x_src) # parsing src_image
    parsing(img=x_fake) # parsing fake_image
    reconstruct(0) # 'styling' 모드
